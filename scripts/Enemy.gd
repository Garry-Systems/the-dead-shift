...
## A enemy: walks toward the player, has health, damages the player on contact,
## and drops an XP gem when it dies.

@export var xp_gem_scene: PackedScene

var _health := Health.new(GameConfig.ENEMY_MAX_HEALTH)
var _target: Player
var _burn_dps := 0.0
...
func _physics_process(delta: float) -> void:
	...
	velocity = dir * GameConfig.ENEMY_MOVE_SPEED
	move_and_slide()
	...
	if global_position.distance_to(_target.global_position) < 40.0:
		_target.take_damage(GameConfig.ENEMY_TOUCH_DAMAGE * delta)
